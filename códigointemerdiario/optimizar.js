...
class TacOptimizer {
    ...
    constantPropagation(program) {
        const constants = new Map(); // var/temp -> valor constante
        const newProgram = [];
        for (const instr of program) {
            ...
            if (instr.op === '=') {
                ...
                const updatedInstr = { op: instr.op, result: instr.result };
                if (arg1 !== undefined)
                    updatedInstr.arg1 = arg1;
                ...
                switch (instr.op) {
                    case '+':
                        resultValue = parseFloat(arg1) + parseFloat(arg2);
                        break;
                    case '-':
                        resultValue = parseFloat(arg1) - parseFloat(arg2);
                        break;
            ...
            const updatedInstr = { op: instr.op };
            if (arg1 !== undefined)
                updatedInstr.arg1 = arg1;
            if (arg2 !== undefined)
                updatedInstr.arg2 = arg2;
            if (instr.result !== undefined)
                updatedInstr.result = instr.result;
            newProgram.push(updatedInstr);
        }
        return newProgram;
    }
    replaceWithConstant(varName, constants) {
        return constants.has(varName) ? constants.get(varName) : varName;
    }
    commonSubexpressionElimination(program) {
        const exprToTemp = new Map(); // chave: 'op arg1 arg2' -> temp
        const newProgram = [];
        for (const instr of program) {
            ...
            if (instr.arg1 !== undefined)
                updatedInstr.arg1 = instr.arg1;
            if (instr.arg2 !== undefined)
                updatedInstr.arg2 = instr.arg2;
            if (instr.result !== undefined)
                updatedInstr.result = instr.result;
            newProgram.push(updatedInstr);
        }
    ...
    deadCodeElimination(program) {
        ...
        for (let i = program.length - 1; i >= 0; i--) {
            ...
            if (!instr)
                continue; // Safe-guard para possível undefined, embora o loop deva prevenir
...
exports.TacOptimizer = TacOptimizer;
//# sourceMappingURL=optimizar.js.map